// Retained three-stage routed-W13 pipeline for the full-proposal scheduler.
// W2 uses the separate dspark_w13 implementation and its own stage count.
#pragma once

#if defined(DSPARK_W13_GROUP_PROBE) || defined(DSPARK_W13_CREW_PROBE) \
    || defined(DSPARK_W13_STAGING_PROBE)
#error "The retained stage3 W13 header excludes attribution probes"
#endif
...
constexpr int kSharedBytesBudget = 227328;
...
__device__ inline void execute_scalar(const Args& args) {
  for (uint32_t item = args.begin; item < args.end; ++item) {
    ...
    const ItemPlan plan = dspark_w13_stage3::item_plan(args, item);
...
struct W13Policy {
  using Args = dspark_w13_stage3::Args;
  ...
  static __device__ __forceinline__ PipelinePlan plan(
    ...
    const ItemPlan resolved = dspark_w13_stage3::item_plan(args, item);
    PipelinePlan view;
...
__device__ inline void execute_tcgen(const Args& args) {
  dspark_w13_stage3::execute_fp4_pipeline<W13Policy>(args);
}
  ...
  if (warp >= 5) {
    ...
    for (uint32_t item = args.begin; item < args.end; ++item) {
      const GroupContext ctx =
          dspark_w13_stage3::group_plan<Policy>(args, item, storage.route_indices, group_cache);
      if (ctx.member_count == 0) {
        continue;
      }
...
__device__ inline void execute_tcgen_grouped(const Args& args) {
  dspark_w13_stage3::execute_fp4_grouped_pipeline<W13GroupPolicy>(args);
}
...
__device__ inline void execute_tcgen_auto(const Args& args) {
  if (dspark_w13_stage3::grouped_profitable<W13GroupPolicy>(args)) {
    dspark_w13_stage3::execute_fp4_grouped_pipeline<W13GroupPolicy>(args);
  } else {
    dspark_w13_stage3::execute_fp4_pipeline<W13Policy>(args);
  }
...
__device__ inline void execute_dg_grouped_pipeline(
  ...
  const auto physical_item_at = [&](uint32_t item) {
    if constexpr (kCompact) {
      ...
      return row * 32 + tile;
    } else return dspark_w13_stage3::dg_physical_item(args, item);
  };
  ...
  const auto resolve_group = [&](uint32_t item, GroupCache& cache) {
    if constexpr (kGroupMasks) return dspark_w13_stage3::masked_group_plan<Policy>(args, item, storage.route_indices, cache);
    else return dspark_w13_stage3::group_plan<Policy>(args, item, storage.route_indices, cache);
  };
  ...
  if (kDedicatedWeights && warp == 5) {
    ...
    for (uint32_t item = item_begin; item < item_end; item = next_item(item)) {
      ...
      const uint8_t* const weight_scales = ctx.weight_scales;
      const int output_base = ctx.output_base;
      const int planes = kCompact ? 1 : dspark_w13_stage3::dg_live_planes(args, item);
#pragma unroll 1
    ...
    auto wide_drain = [&](const GroupContext& ctx, int e, int planes,
      ...
      // Barrier 1, not 0. Barrier 0 is also __syncthreads', and in this build
      // the other four staging warps keep running (and eventually reach the
      // body's closing __syncthreads) while these four are inside the drain,
      // so their arrivals would inflate a `bar.sync 0, 128` count. The frozen
      // crew split gets away with barrier 0 because the drain is thread-local
      // either side of it; the wide build does not rely on that.
...
__device__ inline void execute_w13_dg(const Args& args) {
  ...
  dspark_w13_stage3::execute_dg_grouped_pipeline<W13GroupPolicy, false, false, 0, DSPARK_ROUTED_PACKED_SFA, DSPARK_ROUTED_SF_RING, DSPARK_ROUTED_GROUP_MASKS, DSPARK_ROUTED_REUSE_TMEM, DSPARK_ROUTED_COMPACT_TILES
#ifdef DSPARK_ROUTED_FUSED_SWIGLU
      , true
#endif
      >(args);
#else
  dspark_w13_stage3::execute_dg_grouped_pipeline<W13GroupPolicy>(args);
#endif
}
...
#ifdef DSPARK_ROUTED_PACKED_SFA
  dspark_w13_stage3::execute_dg_grouped_pipeline<W13GroupPolicy, true, false, 0, DSPARK_ROUTED_PACKED_SFA, DSPARK_ROUTED_SF_RING, DSPARK_ROUTED_GROUP_MASKS, DSPARK_ROUTED_REUSE_TMEM, DSPARK_ROUTED_COMPACT_TILES
#ifdef DSPARK_ROUTED_FUSED_SWIGLU
      , true
#endif
      >(args, weight_tma_descriptor);
#else
  dspark_w13_stage3::execute_dg_grouped_pipeline<W13GroupPolicy, true>(
      args, weight_tma_descriptor);
#endif
}
...
__device__ inline void execute_w13_dg_tma_stage_major(
    const Args& args, const void* weight_tma_descriptor) {
  dspark_w13_stage3::execute_dg_grouped_pipeline<W13GroupPolicy, true, true>(
      args, weight_tma_descriptor);
}
...
using W2Args = dspark_w13::W2Args;
...
__device__ inline void execute_w2_scalar(const W2Args& args) {
  for (uint32_t item = args.begin; item < args.end; ++item) {
    ...
    const W2ItemPlan plan = dspark_w13_stage3::w2_item_plan(args, item);
    const int output = plan.output_base + static_cast<int>(threadIdx.x);
    float result = 0.0f;
...
struct W2Policy {
  ...
  static __device__ __forceinline__ PipelinePlan plan(
      const Args& args, uint32_t item) {
    const W2ItemPlan resolved = dspark_w13_stage3::w2_item_plan(args, item);
    PipelinePlan view;
    view.weight = resolved.weight;
    view.weight_scales = resolved.weight_scales;
...
__device__ inline void execute_w2_tcgen(const W2Args& args) {
  dspark_w13_stage3::execute_fp4_pipeline<W2Policy>(args);
}
...
__device__ inline void execute_w2_tcgen_grouped(const W2Args& args) {
  dspark_w13_stage3::execute_fp4_grouped_pipeline<W2GroupPolicy>(args);
}

__device__ inline void execute_w2_tcgen_auto(const W2Args& args) {
  if (dspark_w13_stage3::grouped_profitable<W2GroupPolicy>(args)) {
    dspark_w13_stage3::execute_fp4_grouped_pipeline<W2GroupPolicy>(args);
  } else {
    dspark_w13_stage3::execute_fp4_pipeline<W2Policy>(args);
  }
...
#ifdef DSPARK_ROUTED_PACKED_SFA
  dspark_w13_stage3::execute_dg_grouped_pipeline<W2GroupPolicy, false, false, 0, DSPARK_ROUTED_PACKED_SFA, DSPARK_ROUTED_SF_RING, DSPARK_ROUTED_GROUP_MASKS, DSPARK_ROUTED_REUSE_TMEM, DSPARK_ROUTED_COMPACT_TILES>(args);
#else
  dspark_w13_stage3::execute_dg_grouped_pipeline<W2GroupPolicy>(args);
#endif
}
...
#ifdef DSPARK_ROUTED_PACKED_SFA
  dspark_w13_stage3::execute_dg_grouped_pipeline<W2GroupPolicy, true, false, 0, DSPARK_ROUTED_PACKED_SFA, DSPARK_ROUTED_SF_RING, DSPARK_ROUTED_GROUP_MASKS, DSPARK_ROUTED_REUSE_TMEM, DSPARK_ROUTED_COMPACT_TILES>(
      args, weight_tma_descriptor);
#else
  dspark_w13_stage3::execute_dg_grouped_pipeline<W2GroupPolicy, true>(
      args, weight_tma_descriptor);
#endif
}
...
__device__ inline void execute_w2_dg_tma_stage_major(
    const W2Args& args, const void* weight_tma_descriptor) {
  dspark_w13_stage3::execute_dg_grouped_pipeline<W2GroupPolicy, true, true>(
      args, weight_tma_descriptor);
}
#endif
