// Grouped WO_A split-K prototype. Separate GPU-scheduled compute and
// reduction phases reuse WO_B partial storage before WO_B starts. Splitting
// changes FP32 accumulation order and requires relaxed-numerics validation.
#pragma once
#include "dspark_proj_phase.cuh"

namespace dspark_woa_splitk {
struct Args : dspark_proj::WoaArgs {
  float* partials;
};
...
template <int SlotK, int Splits, class Args>
__device__ inline void execute(const Args& args) {
  ...
  constexpr int kUmmaSlotsPerItem = 4096 / SlotK / Splits;
  ...
  if (warp >= 5) {
    if (threadIdx.x == 160 || warp == 6) {
      for (int c = 0; c < total_slots; ++c) {
        ...
        const int raw_item =
            static_cast<int>(args.begin) + c / kUmmaSlotsPerItem;
        const int split = raw_item % Splits;
        const int item = raw_item / Splits;
        const int slot_in_item = c % kUmmaSlotsPerItem;
        if (threadIdx.x == 160) {
          constexpr int bytes = kUmmaSlotElements * sizeof(__nv_bfloat16);
          const auto* source =
              args.wo_a +
              (static_cast<int64_t>(item) * kUmmaSlotsPerItem * Splits +
               split * kUmmaSlotsPerItem + slot_in_item) *
                  kUmmaSlotElements;
    ...
    for (int i = 0; i < item_count; ++i) {
      ...
      const int64_t tile_base =
          ((static_cast<int64_t>(args.begin) + i) / Splits) * kOutputTile;
      const int split = (args.begin + i) % Splits;
      CUTE_UNROLL
      for (int row = 0; row < kDraftBlock; ++row) {
        args.partials[(static_cast<int64_t>(row) * kVocab + tile_base +
                       threadIdx.x) *
                          Splits +
                      split] =
...
template <int Splits, class Args>
__device__ inline void reduce(const Args& args) {
...
#pragma unroll
      for (int part = 0; part < 4; ++part) {
        const int index = base + lane + part * 32;
        float value = args.partials[index * Splits];
#pragma unroll
        for (int split = 1; split < Splits; ++split)
          value += args.partials[index * Splits + split];
